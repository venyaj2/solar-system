...
namespace solarsystem {
    ...
    const Planet Planet::SUN = Planet ("Sun", 40, "yellow", 0, 0,
                  "Moons: None \nDistance from Sun: 35.98 million mi\nRadius: 1,516 mi\nDiameter:"
                                  " 4,879 km\nOrbital period: 88 days"
                                  "\nMass: 3.285 × 10^23 kg\nSurface Temperature: -173 to 427°C", 0);

    const Planet Planet::MERCURY = Planet("Mercury", 4, "gray", 57, 55, "Radius: 432,690 mi\nDiameter: 1.3927 million km\nMass: 31.989 × 10^30 kg\nSurface Temperature: 5,505°C\nNumber of Moons: None\nFirst Record: 14th century BCE by Assyrian astronomers", 3);
    
    const Planet Planet::VENUS = Planet("Venus", 6, "brown", 87, 70, "Venus", 2);

    const Planet Planet::EARTH = Planet("Earth", 10, "green", 97, 90, "Earth", 1.3);

    const Planet Planet::MARS = Planet("Mars", 8, "red", 137, 115, "Mars", 1);

    const Planet Planet::JUPITER = Planet("Venus", 17, "orange", 200, 240, "Jupiter", .8);

    const Planet Planet::SATURN = Planet("Venus", 14, "yellow", 193, 280, "Saturn", .7);

    const Planet Planet::URANUS = Planet("Uranus", 11, "teal", 280, 315, "Uranus", .3);

    const Planet Planet::NEPTUNE = Planet("Neptune", 10, "blue", 300, 340, "Neptune", .2);
